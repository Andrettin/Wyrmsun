	...
	function() 
		...
		Event(
			...
			Event(
				...
				Event(
					"Rugnur",
					"Well, state your business and be done with it. And if we don't like what you say, you leave!",
					player,
					{"~!Continue"},
					{function(s)
					Event(
						...
						Event(
							"Rugnur",
							"Yes... your deceased king brought it from the west, right? You own it.",
							player,
							{"~!Continue"},
							{function(s)
							Event(
								...
								Event(
									...
									Event(
										...
										Event(
											"Rugnur",
											"And what... you want us to do this, cut the stone? And make an artifact of some kind?",
											player,
											{"~!Continue"},
											{function(s)
											Event(
												...
												Event(
													...
													Event(
														...
														Event(
															"Rugnur",
															"They would refuse to come. You must come into the caves, and go to the city in person, or send a representative.",
															player,
															{"~!Continue"},
															{function(s)
															Event(
																...
																Event(
																	"Rugnur",
																	"Uh, no, wait! Let's talk business - how much will you pay us to do this for you?",
																	player,
																	{"~!Continue"},
																	{function(s)
																	Event(
																		...
																		Event(
																			"Rugnur",
																			"You said you wanted a potent artifact, not a child's playtoy! So... twenty thousand, minimum. The crafting of this will take us many years, you must pay us well.",
																			player,
																			{"~!Continue"},
																			{function(s)
																			Event(
																				...
																				Event(
																					...
																					Event(
																						...
																						Event(
																							"Rugnur",
																							"Fine then - ten thousand... now, what exactly do you want us to make the stone into?",
																							player,
																							{"~!Continue"},
																							{function(s)
																							Event(
...
AddTrigger(
	...
	function() 
		...
		Event(
			...
			Event(
				raider_leader_name,
				"Hey! You can't do that - this road belongs to the " .. wyr.preferences.TheScepterOfFireRaiderFaction .. ". We're not letting you transport gold and jewels on it without paying a toll.",
				player,
				{"~!Continue"},
				{function(s)
				Event(
					"Rugnur",
					"What are you talking about?! This road has always been open to all.",
					player,
					{"~!Continue"},
					{function(s)
					Event(
						raider_leader_name,
						"Yes, we have allowed people to use it, but it is our road.",
						player,
						{"~!Continue"},
						{function(s)
						Event(
							...
							Event(
								raider_leader_name,
								"Nevertheless, if you try to move your caravans here you have to pay the toll, or face our wrath!",
								player,
								{"~!Continue"},
								{function(s)
								Event(
									"Rugnur",
									"Very well. " .. raider_leader_name .. ", I have no problem with slaying you if you stand in our way. I advise you to leave the shipment alone, but not doing so is your choice.",
									player,
									{"~!Continue"},
									{function(s)
										-- create bandits
...
AddTrigger(
	function()
		...
		for i=0,14 do
			if ((GetArrayIncludes(Objectives[i], a_bargain_is_struck_objective_1) or GetArrayIncludes(Objectives[i], "- Have one unit standing on each glyph at the same time")) and (GetPlayerData(i, "UnitTypesCount", "unit-hero-rugnur") + GetPlayerData(i, "UnitTypesCount", "unit-hero-rugnur-older")) < 1) then
				player = i
				return true
			end
		end
	...
	function() 
		...
		if (mapinfo.description == "Chaincolt Foothills" or mapinfo.description == "Caverns of Chaincolt") then
			ActionDefeat()
		end
		return false
	end
...
AddTrigger(
	function()
		...
		for i=0,14 do
			if (GetArrayIncludes(Objectives[i], a_bargain_is_struck_objective_1) and GetPlayerData(i, "UnitTypesCount", "unit-gnomish-recruit") < 1) then
				player = i
				return true
			end
		end
	...
	function() 
		...
		Event(
			...
			Event(
				...
				Event(
					"Rugnur",
					"Yes, that sounds good. I'll stay here and try to defend the gates...",
					player,
					{"~!Continue"},
					{function(s)
					Event(
						closing_the_gates_raider_leader_name,
						"I see you have sent a messenger to the city. Good idea, but will it really help? We have more troops and more provisions, and we are going to enter those caves and kill you!",
						player,
						{"~!Continue"},
						{function(s)
						Event(
							...
							Event(
								...
								Event(
									...
									Event(
										...
										Event(
											"Baglur",
											"Wait! Rugnur, dinna' they tell you anything when you took command here?!",
											player,
											{"~!Continue"},
											{function(s)
											Event(
												"Rugnur",
												"Not really. This was supposedly a time of peace. Who are you?",
												player,
												{"~!Continue"},
												{function(s)
												Event(
													"Baglur",
													"Argh! I'm a retired warrior... it looks like I'll ha' to come out and help you, else we'll be overrun by these damn raiders... Well, listen here. This entrance to the city can be closed. There's a gate. If we can close it, the raiders will no' be able to enter. And you raiders; begone from here!",
													player,
													{"~!Continue"},
													{function(s)
													Event(
														closing_the_gates_raider_leader_name,
														"Cease your meddling, old dwarf! Rugnur is in charge here, and it is he who will bargain with us.",
														player,
														{"~!Continue"},
														{function(s)
														Event(
															"Rugnur",
															"Baglur, how do you close the gates?",
															player,
															{"~!Continue"},
															{function(s)
															Event(
																	...
																	Event(
																		...
																		Event(
																			...
																			Event(
																				...
																				Event(
																					closing_the_gates_raider_leader_name,
																					"Have you made up your mind yet about surrendering? If you will not give up, prepare for battle!",
																					player,
																					{"~!Continue"},
																					{function(s)
																					Event(
																						"Rugnur",
																						"You may be more powerful than us, but I doubt even you can blast through our stonecraft. We refuse - now let's close these gates!",
																						player,
																						{"~!Continue"},
																						{function(s)
	...
	function() 
		...
		Event(
			...
			Event(
				...
				Event(
					closing_the_gates_raider_leader_name,
					"Agh! Well, you have defeated me for now, but eventually you will have to exit these caves, to give " .. wyr.preferences.TheScepterOfFireMonarch .. " back his jewel. And when you do, we will be ready for you.",
					player,
					{"~!Continue"},
					{function(s)
					Event(
							...
							Event(
								...
								Event(
									...
									Event(
										...
										Event(
											"Durstorn",
											"Ha! There hasn't been a border attack since the affair with Pypo I, why would someone attack us now?",
											player,
											{"~!Continue"},
											{function(s)
											Event(
												...
												Event(
													...
													Event(
														"Rugnur",
														"Hey, Durstorn, have we started yet? King " .. wyr.preferences.TheScepterOfFireMonarch .. " came by just now, wanted to talk to you...",
														player,
														{"~!Continue"},
														{function(s)
														Event(
															"Durstorn",
															"Speak properly to your elders, boy! Why are you late?",
															player,
															{"~!Continue"},
															{function(s)
															Event(
																"Rugnur",
																"Huh? Didn't a messenger come and tell you?",
																player,
																{"~!Continue"},
																{function(s)
																Event(
																	...
																	Event(
																		"Baglur",
																		"I think you'd better let me explain. The gnomish king " .. wyr.preferences.TheScepterOfFireMonarch .. " came to the gate. Then we had to close it, which took considerable time.",
																		player,
																		{"~!Continue"},
																		{function(s)
																		Event(
																			"Glinar",
																			"What'd he want? And why'd you have to close the gate?",
																			player,
																			{"~!Continue"},
																			{function(s)
																			Event(
																				...
																				Event(
																					...
																					Event(
																						...
																						Event(
																							"Durstorn",
																							"What?! You already accepted? Foolish boy, you should have come for one of the elders! We could have gotten much more than ten thousand silver out of him! This is a task that will take many years, and now all of our work in that time will bring us very little gain.",
																							player,
																							{"~!Continue"},
																							{function(s)
																							Event(
																								...
																								Event(
																									"Durstorn",
																									"Can't you tell a bluff when you hear one?! He knows we're the best craftsmen, and he wouldn't have gone to another tribe if we stalled for a year. Oh well, what's done is done. We'll have to work for less than I would prefer...",
																									player,
																									{"~!Continue"},
																									{function(s)
																									Event(
																										...
																										Event(
																											...
																											Event(
																												"Baglur",
																												"You mean the insane dwarf who lives in the far north?!",
																												player,
																												{"~!Continue"},
																												{function(s)
																												Event(
																													"Durstorn",
																													"Yes. He is the only one I know of who could craft the scepter the way " .. wyr.preferences.TheScepterOfFireMonarch .. " wants it. And he has nothing to do; he will be fine with wasting 25 years of his life on this task.",
																													player,
																													{"~!Continue"},
																													{function(s)
																													Event(
																														"Rugnur",
																														"Very well, my lord. We will go into the north to find Thursagan. Do you have any idea where he is?",
																														player,
																														{"~!Continue"},
																														{function(s)
																														Event(
...
AddTrigger(
	function()
		...
		for i=0,14 do
			if (GetArrayIncludes(Objectives[i], "- Have one unit standing on each glyph at the same time") and GetPlayerData(i, "UnitTypesCount", "unit-hero-baglur") < 1) then
				player = i
				return true
			end
		end
