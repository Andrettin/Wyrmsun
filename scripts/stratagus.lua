...
function GetNumRivals(player)
	local rival_count = 0
	for i=0,(PlayerMax - 2) do
		if (player ~= i and (CPlayer:GetPlayer(i).Type == PlayerPerson or CPlayer:GetPlayer(i).Type == PlayerComputer) and (Players[player]:IsAllied(CPlayer:GetPlayer(i)) == false or CPlayer:GetPlayer(i):IsAllied(Players[player]) == false) and (GetPlayerData(i, "Faction") == "" or GetFactionData(GetPlayerData(i, "Faction"), "Type") == "tribe" or GetFactionData(GetPlayerData(i, "Faction"), "Type") == "polity") and GetPlayerData(player, "HasContactWith", i)) then
...
function GetNumAllies(player)
	local ally_count = 0
	for i=0,(PlayerMax - 2) do
		if (player ~= i and (CPlayer:GetPlayer(i).Type == PlayerPerson or CPlayer:GetPlayer(i).Type == PlayerComputer) and Players[player]:IsAllied(CPlayer:GetPlayer(i)) and CPlayer:GetPlayer(i):IsAllied(Players[player])) then
