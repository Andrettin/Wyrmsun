...
if (LoadedGame == false) then
	...
	elseif (GrandStrategyEventMap) then
		...
		for i, unitName in ipairs(Units) do
			if (IsMilitaryUnit(unitName)) then
				units_to_be_created[string.gsub(unitName, "-", "_")] = 0
				units_to_be_created[string.gsub(unitName, "-", "_")] = math.floor(WorldMapProvinces.Bavaria.Units[string.gsub(unitName, "-", "_")] / 4)
				WorldMapProvinces.Bavaria.Units[string.gsub(unitName, "-", "_")] = WorldMapProvinces.Bavaria.Units[string.gsub(unitName, "-", "_")] - units_to_be_created[string.gsub(unitName, "-", "_")]
			end
		end
		for i, unitName in ipairs(Units) do
			if (IsMilitaryUnit(unitName)) then
