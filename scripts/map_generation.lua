...
function GenerateRoughLand(rough_land_seed_number, rough_land_expansions_number)
	...
	while (Count > 0 and WhileCount < rough_land_seed_number * 100) do
		RandomX = SyncRand(CMap:get().Info.MapWidth)
		RandomY = SyncRand(CMap:get().Info.MapHeight)
		if (RawTile(RandomX, RandomY) == "Land") then
			RandomNumber = SyncRand(4)
			...
			for sub_y=0,(GetUnitTypeData("unit_gold_deposit", "TileHeight") - 1) do
				if (SyncRand(100) <= 50) then -- give a chance of a gold rock not being generated, to make the shape of the gold rock group seem more natural
					...
					if (symmetric) then
						...
						unit = CreateUnit(rock_type, PlayerNumNeutral, {mirrored_tile_x - (GetUnitTypeData("unit_gold_deposit", "TileWidth") - 1) + sub_x, gold_mine_spawn_point[2] + sub_y})

						unit = CreateUnit(rock_type, PlayerNumNeutral, {gold_mine_spawn_point[1] + sub_x, mirrored_tile_y - (GetUnitTypeData("unit_gold_deposit", "TileHeight") - 1) + sub_y})

						unit = CreateUnit(rock_type, PlayerNumNeutral, {mirrored_tile_x - (GetUnitTypeData("unit_gold_deposit", "TileWidth") - 1) + sub_x, mirrored_tile_y - (GetUnitTypeData("unit_gold_deposit", "TileHeight") - 1) + sub_y})
					end
				end
			end
			Count = Count - 1
...
function CreateCoalMines(coal_mine_number, coal_quantity, min_x, max_x, min_y, max_y, symmetric)
	if (LoadedGame == false) then
		...
		while (Count > 0) do
			...
			unit = CreateUnit("unit_coal_deposit", PlayerNumNeutral, {coal_mine_spawn_point[1], coal_mine_spawn_point[2]})
			...
			if (symmetric) then
				...
				unit = CreateUnit("unit_coal_deposit", PlayerNumNeutral, {mirrored_tile_x - (GetUnitTypeData("unit_coal_deposit", "TileWidth") - 1), coal_mine_spawn_point[2]})
				SetResourcesHeld(unit, coal_quantity)
				Count = Count - 1

				unit = CreateUnit("unit_coal_deposit", PlayerNumNeutral, {coal_mine_spawn_point[1], mirrored_tile_y - (GetUnitTypeData("unit_coal_deposit", "TileHeight") - 1) })
				SetResourcesHeld(unit, coal_quantity)
				Count = Count - 1

				unit = CreateUnit("unit_coal_deposit", PlayerNumNeutral, {mirrored_tile_x - (GetUnitTypeData("unit_coal_deposit", "TileWidth") - 1), mirrored_tile_y - (GetUnitTypeData("unit_coal_deposit", "TileHeight") - 1) })
				SetResourcesHeld(unit, coal_quantity)
				Count = Count - 1
			end
		end
...
function CreateStartingLocationResourcePiles(loc_player, pile_type, pile_quantity)
	if (LoadedGame == false) then
		...
		local max_x = math.min(CPlayer:GetPlayer(loc_player).StartPos.x + 8, CMap:get().Info.MapWidth - 1)
		local min_y = math.max(CPlayer:GetPlayer(loc_player).StartPos.y - 8, 0)
		local max_y = math.min(CPlayer:GetPlayer(loc_player).StartPos.y + 8, CMap:get().Info.MapHeight - 1)
...
function CreateCritters(arg)
	...
	if (LoadedGame == false) then
		local critter_number = (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 256
		local RandomNumber = 0
		local RandomX = 0
		local RandomY = 0
		...
		while (Count > 0 and WhileCount < critter_number * 1000) do
			...
			for sub_x = -(GetUnitTypeData(critter_unit_type, "TileWidth")), GetUnitTypeData(critter_unit_type, "TileWidth") do
				for sub_y = -(GetUnitTypeData(critter_unit_type, "TileHeight")), GetUnitTypeData(critter_unit_type, "TileHeight") do
					if (CMap:get().Info:IsPointOnMap(RandomX + sub_x, RandomY + sub_y, 0)) then
						if not (
							(
								(
...
function CreateCreeps(player, creep_type, creep_number, min_x, min_y, max_x, max_y)
	if (LoadedGame == false) then
		...
		max_x = math.min(max_x, CMap:get().Info.MapWidth - 1)
		min_y = math.max(min_y, 0)
		max_y = math.min(max_y, CMap:get().Info.MapHeight - 1)
...
function CreateRoamingFog(fog_number)
	if (LoadedGame == false) then
		...
		while (Count > 0 and WhileCount < fog_number * 100) do
			...
			RandomX = SyncRand(CMap:get().Info.MapWidth - GetUnitTypeData(fog_type, "TileWidth"))
			RandomY = SyncRand(CMap:get().Info.MapHeight - GetUnitTypeData(fog_type, "TileHeight"))
			if (GetTileTerrainHasFlag(RandomX, RandomY, "air-unpassable") == false and GetUnitTypeData(fog_type, "TileWidth") < CMap:get().Info.MapWidth - RandomX - 1 and GetUnitTypeData(fog_type, "TileHeight") < CMap:get().Info.MapHeight - RandomY - 1) then
...
function CreateDecorations()
	if (LoadedGame == false) then
		...
		Count = (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 128
		if (decoration_count == 0 and GetTileTerrainFlagCount("land") > Count) then
			while (Count > 0 and WhileCount < ((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 128) * 100) do
				RandomX = SyncRand(CMap:get().Info.MapWidth)
				RandomY = SyncRand(CMap:get().Info.MapHeight)
				...
				elseif (GetTileTerrainName(RandomX, RandomY) == "dwarf-closed-wall" and RandomY < (CMap:get().Info.MapHeight - 1) and GetNumUnitsAt(PlayerNumNeutral, "any", {RandomX, RandomY}, {RandomX, RandomY}) < 1) then
	...
	for i=0,(PlayerMax - 2) do
		if (CMap:get().Info.PlayerType[i] == PlayerPerson or CMap:get().Info.PlayerType[i] == PlayerComputer) then
			...
			if (table.getn(possible_civilizations) < 1) then
				...
				if (CMap:get().Info.PlayerType[i] ~= PlayerNobody) then
					...
					if (CPlayer:GetPlayer(i).StartPos.x > 0 and CPlayer:GetPlayer(i).StartPos.y > 0) then -- if the player already has a custom starting location set, then see if that area (or failing that the one surrounding it) is actually free
						...
						if (symmetric == false) then
							...
							elseif (i == 1 or i == 5 or i == 9 or i == 13) then
								player_spawn_point = {math.abs(symmetric_starting_location[1] - (CMap:get().Info.MapWidth - 1)) - 3, math.abs(symmetric_starting_location[2] - (CMap:get().Info.MapHeight - 1)) - 3}
							elseif (i == 2 or i == 6 or i == 10 or i == 14) then
								player_spawn_point = {symmetric_starting_location[1], math.abs(symmetric_starting_location[2] - (CMap:get().Info.MapHeight - 1)) - 3}
							elseif (i == 3 or i == 7 or i == 11) then
								player_spawn_point = {math.abs(symmetric_starting_location[1] - (CMap:get().Info.MapWidth - 1)) - 3, symmetric_starting_location[2]}
							end
						end
					end

					starting_point_found = true
					if ((player_spawn_point[1] + 4) > (CMap:get().Info.MapWidth - 1) or (player_spawn_point[2] + 4) > (CMap:get().Info.MapHeight - 1) or (player_spawn_point[1] - 1) < 0 or (player_spawn_point[2] - 1) < 0) then
						starting_point_found = false
					end
					for j=0,(PlayerMax - 2) do
						if (j < i and (CMap:get().Info.PlayerType[j] == PlayerPerson or CMap:get().Info.PlayerType[j] == PlayerComputer)) then
...
function SetMapBorders(tile_type, replace_tile)
	...
	for x=0,(CMap:get().Info.MapWidth - 1) do
		...
		if (RawTile(x, CMap:get().Info.MapHeight - 2) == "" or replace_tile == true) then
			SetRawTile(x, CMap:get().Info.MapHeight - 2, tile_type)
		end
		if (RawTile(x, CMap:get().Info.MapHeight - 1) == "" or replace_tile == true) then
			SetRawTile(x, CMap:get().Info.MapHeight - 1, tile_type)
		end
	end
	for y=0,(CMap:get().Info.MapHeight - 1) do
		...
		if (RawTile(CMap:get().Info.MapWidth - 2, y) == "" or replace_tile == true) then
			SetRawTile(CMap:get().Info.MapWidth - 2, y, tile_type)
		end
		if (RawTile(CMap:get().Info.MapWidth - 1, y) == "" or replace_tile == true) then
			SetRawTile(CMap:get().Info.MapWidth - 1, y, tile_type)
		end
	end
end
...
function GenerateRandomMap(arg)
	if (LoadedGame == false) then
		...
		if (arg.WaterMap) then
			...
			if (arg.LandQuantity == "high") then
				GenerateLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 2, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
			elseif (arg.LandQuantity == "medium") then
				GenerateLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 4, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
			elseif (arg.LandQuantity == "low") then
				GenerateLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 8, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
			end
		else
			for x=0,(CMap:get().Info.MapWidth - 1) do
				for y=0,(CMap:get().Info.MapHeight - 1) do
		...
		if (arg.WaterLayout ~= nil) then
			local block_size = 16
			if (CMap:get().Info.MapWidth > CMap:get().Info.MapHeight) then
				block_size = CMap:get().Info.MapWidth / table.getn(arg.WaterLayout[1])
			else
				block_size = CMap:get().Info.MapHeight / table.getn(arg.WaterLayout)
			end
		...
		if (arg.WaterQuantity == "high") then
			GenerateWater((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 8, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		elseif (arg.WaterQuantity == "medium") then
			GenerateWater((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 16, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		elseif (arg.WaterQuantity == "low") then
			GenerateWater((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		end
		...
		if (arg.RockQuantity == "high") then
			GenerateRocks((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, ((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 8), "Land", 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		elseif (arg.RockQuantity == "medium") then
			GenerateRocks((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, ((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 16), "Land", 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		...
		if (arg.StartingLocationLayout ~= nil) then
			...
			local block_size = 16
			if (CMap:get().Info.MapWidth > CMap:get().Info.MapHeight) then
				block_size = CMap:get().Info.MapWidth / table.getn(arg.StartingLocationLayout[1])
			else
				block_size = CMap:get().Info.MapHeight / table.getn(arg.StartingLocationLayout)
			end
		...
		CreatePlayers(0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight, not arg.NoTownHall, symmetric, not arg.NoDeposits, arg.PlayerCivilizations, arg.PlayerBuildings, arg.WaterMap) -- generate players after rocks and water
		...
		elseif (arg.RoughLandQuantity == "medium") then
			GenerateRoughLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 16)
		elseif (arg.RoughLandQuantity == "low") then
			GenerateRoughLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32)
		end

		if (arg.DarkRoughLandQuantity == "high") then
			GenerateDarkRoughLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 256, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight, "Rough")
		...
		elseif (arg.DarkRoughLandQuantity == "low") then
			GenerateDarkRoughLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 256, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight, "Rough")
		end

		if (arg.TreeQuantity == "high") then
			GenerateTrees((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 512, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 8, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		elseif (arg.TreeQuantity == "medium") then
			GenerateTrees((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 16, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		elseif (arg.TreeQuantity == "low") then
			GenerateTrees((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 2048, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		end

		if (arg.DarkLandQuantity == "high") then
			GenerateDarkLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 256, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		...
		elseif (arg.DarkLandQuantity == "low") then
			GenerateDarkLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 256, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		end

		if (arg.DarkWaterQuantity == "high") then
			GenerateDarkWater((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 256, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		elseif (arg.DarkWaterQuantity == "medium") then
			GenerateDarkWater((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 512, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 64, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		elseif (arg.DarkWaterQuantity == "low") then
			GenerateDarkWater((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 128, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
		end

		if (symmetric) then
			for x=(CMap:get().Info.MapWidth / 2),(CMap:get().Info.MapWidth - 1) do
				for y=0,(CMap:get().Info.MapHeight - 1) do
			...
			for x=0,(CMap:get().Info.MapWidth - 1) do
				for y=(CMap:get().Info.MapHeight / 2),(CMap:get().Info.MapHeight - 1) do
		...
		if (arg.NoDeposits) then
			CreateGoldRocks((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 2048, 0, CMap:get().Info.MapWidth - 3, 0, CMap:get().Info.MapHeight - 3, symmetric)
		else
			CreateGoldSpots((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 2048, 0, CMap:get().Info.MapWidth - 3, 0, CMap:get().Info.MapHeight - 3, symmetric)
		end

		if (arg.MercenaryCamp) then
			CreateNeutralBuildings("unit-mercenary-camp", 1, 0, CMap:get().Info.MapWidth - 3, 0, CMap:get().Info.MapHeight - 3, symmetric)
		end
	...
	--	end
		...
		if (GetCurrentTileset() == "swamp") then
			CreateNeutralBuildings("unit-yale-cave", (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 4096, 0, CMap:get().Info.MapWidth - 3, 0, CMap:get().Info.MapHeight - 3, symmetric)
		end
		...
		if (GetCurrentTileset() == "swamp" or GetCurrentTileset() == "conifer-forest-summer" or GetCurrentTileset() == "conifer-forest-autumn" or GetCurrentTileset() == "fairlimbed-forest") then
			CreateNeutralBuildings("unit-tree-stump", (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 4096, 0, CMap:get().Info.MapWidth - 2, 0, CMap:get().Info.MapHeight - 2, symmetric)
		elseif (GetCurrentTileset() == "cave") then
			CreateNeutralBuildings("unit-hole", (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 4096, 0, CMap:get().Info.MapWidth - 2, 0, CMap:get().Info.MapHeight - 2, symmetric)
		end
...
function SetRawTile(x, y, tile_type)
	...
	if (x >= 0 and x < CMap:get().Info.MapWidth and y >= 0 and y < CMap:get().Info.MapHeight) then
...
function RawTile(x, y)
	...
	elseif (x < 0 and y >= 0 and y < CMap:get().Info.MapHeight and RawTiles[y+1][0+1] ~= "Wall") then
		return RawTiles[y+1][0+1]
	elseif (x >= CMap:get().Info.MapWidth and y >= 0 and y < CMap:get().Info.MapHeight and RawTiles[y+1][CMap:get().Info.MapWidth] ~= "Wall") then
		return RawTiles[y+1][CMap:get().Info.MapWidth]
	elseif (x >= 0 and x < CMap:get().Info.MapWidth and y < 0 and RawTiles[0+1][x+1] ~= "Wall") then
		return RawTiles[0+1][x+1]
	elseif (x >= 0 and x < CMap:get().Info.MapWidth and y >= CMap:get().Info.MapHeight and RawTiles[CMap:get().Info.MapHeight][x+1] ~= "Wall") then
		return RawTiles[CMap:get().Info.MapHeight][x+1]
	elseif (x < 0 and y < 0 and RawTiles[0+1][0+1] ~= "Wall") then
		return RawTiles[0+1][0+1]
	elseif (x >= CMap:get().Info.MapWidth and y < 0 and RawTiles[0+1][CMap:get().Info.MapWidth] ~= "Wall") then
		return RawTiles[0+1][CMap:get().Info.MapWidth]
	elseif (x < 0 and y >= CMap:get().Info.MapHeight and RawTiles[CMap:get().Info.MapHeight][0+1] ~= "Wall") then
		return RawTiles[CMap:get().Info.MapHeight][0+1]
	elseif (x >= CMap:get().Info.MapWidth and y >= CMap:get().Info.MapHeight and RawTiles[CMap:get().Info.MapHeight][CMap:get().Info.MapWidth] ~= "Wall") then
		return RawTiles[CMap:get().Info.MapHeight][CMap:get().Info.MapWidth]
	else
		return ""
	end
end
...
function SpreadTiles(x1, y1, x2, y2, src, des)
	...
	ReplaceTiles(0, 0, CMap:get().Info.MapWidth - 1, CMap:get().Info.MapHeight - 1, "Replace", src)
end
...
function CreateStartingGoldMine(player, x, y, no_raw_tile)
	...
	while (gold_mine_built == false and WhileCount < 100) do
		...
		if ((gold_mine_spawn_point[1] + 2) > CMap:get().Info.MapWidth or (gold_mine_spawn_point[2] + 2) > CMap:get().Info.MapHeight or gold_mine_spawn_point[1] < 0 or gold_mine_spawn_point[2] < 0) then
			free_square = false
		end
		if (free_square) then
			if (no_raw_tile) then
	...
	while (building_built == false and WhileCount < 10000) do
		...
		for sub_x=-1,width+1 do
			for sub_y=-1,height+1 do
					...
					if (building_spawn_point[1] + sub_x < 0 or building_spawn_point[1] + sub_x >= CMap:get().Info.MapWidth or building_spawn_point[2] + sub_y < 0 or building_spawn_point[2] + sub_y >= CMap:get().Info.MapHeight) then
						free_square = false
					end
			end
		end
...
function GenerateValley(direction, lake_quantity)
	...
	FillArea(0, 0, (CMap:get().Info.MapWidth - 1), (CMap:get().Info.MapHeight - 1), "Land", false)
	
	if (direction == "north-south") then
		CreatePlayers(round(CMap:get().Info.MapWidth / 6), round(CMap:get().Info.MapWidth * 5 / 6), 0, CMap:get().Info.MapHeight, true, false, true, nil, nil, false)
		
		GenerateRocks(((CMap:get().Info.MapWidth / 6 * CMap:get().Info.MapHeight) / 32), ((CMap:get().Info.MapWidth / 6 * CMap:get().Info.MapHeight) / 4), "Land", 0, round(CMap:get().Info.MapWidth / 6), 0, CMap:get().Info.MapHeight)
		
		GenerateRocks(((CMap:get().Info.MapWidth / 6 * CMap:get().Info.MapHeight) / 32), ((CMap:get().Info.MapWidth / 6 * CMap:get().Info.MapHeight) / 4), "Land", round(CMap:get().Info.MapWidth * 5 / 6), CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)

		GenerateWater(lake_quantity, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 16, round(CMap:get().Info.MapWidth / 6), round(CMap:get().Info.MapWidth * 5 / 6), 0, CMap:get().Info.MapHeight)
	elseif (direction == "west-east") then
		CreatePlayers(0, CMap:get().Info.MapWidth, round(CMap:get().Info.MapHeight / 6), round(CMap:get().Info.MapHeight * 5 / 6), true, false, true, nil, nil, false)
		
		GenerateRocks(((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight / 6) / 32), ((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight / 6) / 4), "Land", 0, CMap:get().Info.MapWidth, 0, round(CMap:get().Info.MapHeight / 6))
		
		GenerateRocks(((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight / 6) / 32), ((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight / 6) / 4), "Land", 0, CMap:get().Info.MapWidth, round(CMap:get().Info.MapHeight * 5 / 6), CMap:get().Info.MapHeight)
		
		GenerateWater(lake_quantity, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 16, 0, CMap:get().Info.MapWidth, round(CMap:get().Info.MapHeight / 6), round(CMap:get().Info.MapHeight * 5 / 6))
	end
...
--	GenerateRoughLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 8)
	...
	GenerateTrees((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 16, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
...
function GenerateRandomDungeon(player_civilization, player_name, player_hero, second_player_civilization, second_player_name, second_player_hero, hostile_dungeon_player_civilization, hostile_dungeon_player_name, hostile_creep_number, dungeon_boss, dungeon_boss_name, second_dungeon_boss, second_dungeon_boss_name, boss_room_decorations, passive_dungeon_player_name, passive_dweller_number)
	...
	--				GenerateDarkLand(1, SyncRand(6), min_x, max_x, min_y, max_y)
			...
			GenerateDarkRoughLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 512, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 256, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight, "Land")
			...
			Count = 1
			if (CMap:get().Info.PlayerType[1] == PlayerNobody) then
				Count = 0
			end
			WhileCount = 0
			while (Count > 0 and WhileCount < 10000) do
							...
							if (GetNumUnitsAt(0, "any", {RandomX - 4, RandomY - 4}, {RandomX + 4, RandomY + 4}) < 1 and GetNumUnitsAt(1, "any", {RandomX - 4, RandomY - 4}, {RandomX + 4, RandomY + 4}) < 1) then
								...
								if (adjacent_floor_tiles >= 8) then
									if (GetNumUnitsAt(-1, "any", {RandomX, RandomY}, {RandomX, RandomY}) < 1) then
										...
										elseif (hostile_dungeon_player_civilization == "kobold") then
											if (GetNumUnitsAt(-1, "unit-long-swordsman", {0, 0}, {MaxMapWidth, MaxMapHeight}) < 1) then -- there is a small chance that a neutral long swordsman will be created (only one per map)
												unit = OldCreateUnit("unit-long-swordsman", 3, {RandomX, RandomY})
												SetUnitVariable(unit, "Active", false)
												Count = Count - 1
											else
...
function GenerateCave(town_halls, symmetric)
	...
	CreatePlayers(16, CMap:get().Info.MapWidth - 16, 16, CMap:get().Info.MapHeight - 16, town_halls, symmetric, true, nil, nil, false)

	GenerateRocks(((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024),  ((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 4), "Land", 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)

	GenerateWater((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 64, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)

	GenerateRoughLand((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 4096, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32)

	GenerateTrees((CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 1024, (CMap:get().Info.MapWidth * CMap:get().Info.MapHeight) / 32, 0, CMap:get().Info.MapWidth, 0, CMap:get().Info.MapHeight)
...
function FindNearestTerrainTile(terrain, starting_x, starting_y)
	...
	while (search_range < CMap:get().Info.MapWidth and search_range < CMap:get().Info.MapHeight) do
