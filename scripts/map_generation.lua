...
function CreateStartingLocationResourcePiles(loc_player, pile_type, pile_quantity)
	if (LoadedGame == false) then
		...
		local max_x = math.min(GetPlayerData(loc_player, "StartPosX") + 8, Map.Info.MapWidth - 1)
		local min_y = math.max(GetPlayerData(loc_player, "StartPosY") - 8, 0)
		local max_y = math.min(GetPlayerData(loc_player, "StartPosY") + 8, Map.Info.MapHeight - 1)
			...
			for i=0,(PlayerMax - 2) do
				if (Map.Info.PlayerType[i] == PlayerPerson or Map.Info.PlayerType[i] == PlayerComputer) then
					for j=1,arg.WorkerQuantity do
						unit = CreateUnit("unit-germanic-worker", i, {GetPlayerData(i, "StartPosX"), GetPlayerData(i, "StartPosY")})
					end
				end
			end
		end
