...
AddTrigger("the-journey-to-godheim", -- Source: Snorri Sturlson, "Heimskringla", 1844, vol. 1, p. 227.
	function()
		...
		for i=0,(PlayerMax - 2) do
			if (GetPlayerData(i, "TotalNumUnitsConstructed") > 0 and SyncRand(100) == 0 and (GetPlayerData(i, "Faction") == "yngling-tribe" or GetPlayerData(i, "Faction") == "swedish_tribe")) then -- should also check if the faction has an appropriate hero
				trigger_player = i
				return true
			end
		end
...
AddTrigger("marauding-finland", -- Source: Snorri Sturlson, "Heimskringla", 1844, vol. 1, p. 233.
	function()
		...
		for i=0,(PlayerMax - 2) do
			if (GetPlayerData(i, "TotalNumUnitsConstructed") > 0 and SyncRand(100) == 0 and (GetPlayerData(i, "Faction") == "yngling-tribe" or GetPlayerData(i, "Faction") == "swedish_tribe")) then
				trigger_player = i
				return true
			end
		end
