	...
	function()
		for i=0,(PlayerMax - 2) do
			if (GetPlayerData(i, "TotalNumUnitsConstructed") > 0 and FindHero("erala", i) ~= nil and GetFactionExists("hersing-tribe") and GetPlayerData(i, "HasQuest", "heimdalls-progeny") and GetPlayerData(i, "HasQuest", "neutralize-the-hersings")) then
				local erala_hero_unit = FindHero("erala", i)
				if (GetNumUnitsAt(GetFactionPlayer("hersing-tribe"), "units", {GetUnitVariable(erala_hero_unit, "PosX") - 3, GetUnitVariable(erala_hero_unit, "PosY") - 3}, {GetUnitVariable(erala_hero_unit, "PosX") + 3, GetUnitVariable(erala_hero_unit, "PosY") + 3}, GetUnitVariable(erala_hero_unit, "MapLayer")) > 0 and GetPlayerData(GetFactionPlayer("hersing-tribe"), "IsEnemy", i) == false) then
					trigger_player = i
					return true
				end
			end
	...
	function()
		for i=0,(PlayerMax - 2) do
			if (
				...
				and (GetNumUnitsAt(-2, "any", {4023 - EarthStartX, 733 - EarthStartY}, {4087 - EarthStartX, 829 - EarthStartY}, GetMapLayer("material-plane", "earth", 0)) - GetNumUnitsAt(i, "any", {4023 - EarthStartX, 733 - EarthStartY}, {4087 - EarthStartX, 829 - EarthStartY}, GetMapLayer("material-plane", "earth", 0))) == 0
			) then
				trigger_player = i
				return true
			end
