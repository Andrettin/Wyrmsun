...
if (LoadedGame == false) then
	...
	CreateCoalMines(1, 12500, CMap:get().Info.MapWidth / 2, CMap:get().Info.MapWidth - 3, CMap:get().Info.MapHeight / 2, CMap:get().Info.MapHeight - 3, false)
	CreateCoalMines(1, 12500, 0, CMap:get().Info.MapWidth / 2, 0, CMap:get().Info.MapHeight / 2, false)
end
...
AddTrigger("gathering-materials-introduction",
	function()
		...
		if (PlayerHasObjective(GetThisPlayer(), "- Collect 10000 copper and 20000 coal") == false and GetNumUnitsAt(-1, "unit_gold_deposit", {0, 0}, {MaxMapWidth, MaxMapHeight}) >= 1 and GetNumUnitsAt(-1, "unit-coal-deposit", {0, 0}, {MaxMapWidth, MaxMapHeight}) >= 2) then
