...
AddTrigger("grafvitning-kobolds-send-attacker",
	function()
		...
		for i=0,(PlayerMax - 2) do
			if (GetPlayerData(i, "TotalNumUnitsConstructed") > 0 and GetPlayerData(i, "Faction") == "grafvitning-tribe" and GetPlayerData(i, "UnitTypesAiActiveCount", "unit-kobold-footpad") >= 1 and FindHero("modsognir") ~= nil and FindHero("durin") ~= nil and FindHero("modsognir", i) == nil and FindHero("durin", i) == nil) then
				...
				if (GetPlayerData(modsognir_player, "Type") ~= PlayerNeutral and GetPlayerData(modsognir_player, "UnitTypesCount", "unit-dwarven-barracks") > 0) then
					trigger_player = i
					return true
				end
			end
	...
	function()
		...
		local attacker_x = GetUnitVariable(attacker_unit, "PosX")
		local attacker_y = GetUnitVariable(attacker_unit, "PosY")
		...
		for unit1 = 1,table.getn(uncount) do 
			if (uncount[unit1] and GetUnitTypeData(GetUnitVariable(uncount[unit1], "Ident"), "Building")) then
				if (modsognir_building == nil) then
					...
					local old_pos_difference = math.abs(GetUnitVariable(modsognir_building, "PosX") - attacker_x) + math.abs(GetUnitVariable(modsognir_building, "PosY") - attacker_y)
					local new_pos_difference = math.abs(GetUnitVariable(uncount[unit1], "PosX") - attacker_x) + math.abs(GetUnitVariable(uncount[unit1], "PosY") - attacker_y)
