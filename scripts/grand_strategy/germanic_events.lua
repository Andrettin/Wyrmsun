			...
			function(s)
				if (GrandStrategyFaction ~= nil and GrandStrategyFaction.Name == "asa-tribe" and wyr.preferences.AutomaticBattles == false) then
					...
					if (GameResult == GameVictory) then
						ChangeFactionResource("germanic", "asa-tribe", "copper", 1000) -- copper from raiding Vanaland
						ChangeFactionResource("celt", "vana-tribe", "copper", -1000) -- copper lost from the raid
					...
					for i, unitName in ipairs(Units) do
						if (IsOffensiveMilitaryUnit(unitName)) then
							ChangeProvinceUnitQuantity("Astrakhan", unitName, math.ceil(GetPlayerData(0, "UnitTypesStartingNonHeroCount", unitName) / BattalionMultiplier))
							ChangeProvinceUnitQuantity("Don", unitName, math.ceil(GetPlayerData(1, "UnitTypesStartingNonHeroCount", unitName) / BattalionMultiplier))
						end
					end
			...
			function(s)
				if (GrandStrategyFaction ~= nil and GrandStrategyFaction.Name == "vana-tribe" and wyr.preferences.AutomaticBattles == false) then
					...
					if (GameResult == GameDefeat) then
						ChangeFactionResource("germanic", "asa-tribe", "copper", 1000) -- copper from raiding Vanaland
						ChangeFactionResource("celt", "vana-tribe", "copper", -1000)
					end
	...
	WestwardMigration = {
		...
		OptionEffects = {
			...
			function(s) -- if refused to migrate, then a part of the tribe splits and does so
				...
				SetFactionResource(Factions.yngling_tribe.Civilization, "yngling-tribe", "copper", GetFactionResource(EventFaction.Civilization, EventFaction.Name, "copper") / 2)
				ChangeFactionResource(EventFaction.Civilization, EventFaction.Name, "copper", - GetFactionResource(Factions.yngling_tribe.Civilization, "yngling-tribe", "copper"))
				ChangeFactionResource(Factions.yngling_tribe.Civilization, "yngling-tribe", "copper", 2000) -- help them stay afloat a bit
			end
		}
	},
	NorthwardsToTheSea = {
