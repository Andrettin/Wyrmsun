	...
	OptionEffects = {
		function(s)
			...
			if not (found_faction) then
				for key, value in pairs(Factions) do
					if (Factions[key].Civilization == "finnish" and GetFactionData(Factions[key].Civilization, Factions[key].Name, "Type") == GetFactionData(EventFaction.Civilization, EventFaction.Name, "Type") and GetFactionProvinceCount(Factions[key]) == 0) then
						FormFactionLua(EventFaction, Factions[key])
						EventFaction = Factions[key]
						break
					end
		...
		function(s)
			...
			for province_i, province_key in ipairs(EventFaction.OwnedProvinces) do
				if (
					...
					for key, value in pairs(Factions) do
						if (Factions[key].Civilization == "magyar" and GetFactionData(Factions[key].Civilization, Factions[key].Name, "Type") == GetFactionData(EventFaction.Civilization, EventFaction.Name, "Type") and ProvinceHasClaim(WorldMapProvinces[province_key].Name, Factions[key].Civilization, Factions[key].Name) and GetFactionProvinceCount(Factions[key]) == 0) then
							FormFactionLua(EventFaction, Factions[key])
							EventFaction = Factions[key]
							found_faction = true
			...
			if not (found_faction) then
				for key, value in pairs(Factions) do
					if (Factions[key].Civilization == "magyar" and GetFactionData(Factions[key].Civilization, Factions[key].Name, "Type") == GetFactionData(EventFaction.Civilization, EventFaction.Name, "Type") and GetFactionProvinceCount(Factions[key]) == 0) then
						FormFactionLua(EventFaction, Factions[key])
						EventFaction = Factions[key]
						break
					end
